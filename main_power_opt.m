...
for cnt_t = 1:length(T_vec)
        T = T_vec(cnt_t);
        for cnt_k = 1:length(K_vec)
            K = K_vec(cnt_k);
            for cnt_SINR_case = 1:length(SINR_th_db_vec)
                    rng(1234);


                    SINR_th_db = SINR_th_db_vec(cnt_SINR_case);
                    scenario_name = ['M32_power_allocation_T',int2str(T),'_K', int2str(K),'_SINRth',int2str(SINR_th_db)];
                    sensing_optimal_power_simulation(global_variables,T,K,SINR_th_db,scenario_name);
            end
