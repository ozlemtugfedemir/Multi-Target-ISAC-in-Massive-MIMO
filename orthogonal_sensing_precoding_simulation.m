function orthogonal_sensing_precoding_simulation(global_variables,T,K,SINR_th_db,scenario_name);
...
while tot_sim<200
    load(['Setups/',scenario_type,'_',int2str(tot_sim),'.mat'])
    output_parameters = generate_orthogonal_input_params(global_variables,setup_output);
    optimal_outputs = orthogonal_sensing_precoding_min_trace(global_variables, output_parameters);
    
    if optimal_outputs.status == 1
        verification_output  = compare_orhogonal_sensing_precoding_correctness(optimal_outputs, output_parameters, global_variables);
        cramer_output = calculate_crb_matr_for_orthogonal(optimal_outputs, output_parameters, global_variables);
        ...
        num_sim = num_sim+1
        sim_status = 1;
        save(['revised_', scenario_name,'_',int2str(tot_sim)],'SINRS', 'positions','CRB_vec','SNR_tars', 'objectives', 'p' , 'R_s', 'global_variables','sim_status', 'normalized_CRB_vec', 'elapsed_time', 'D');
